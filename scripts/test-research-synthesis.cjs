...
assert.throws(()=>syn.completeDossier(d.id),/non-unresolved claim needs at least one support/i,{cause:undefined});
// The only unsupported claim is explicitly unresolved, so completion is allowed.
