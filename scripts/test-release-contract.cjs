...
assert.equal(candidate.phase,'V1.1-release-certification');
assert.equal(candidate.status,'release-candidate');
...
assert.equal(candidate.releaseCandidateSourceHead,null,'RC source head is frozen only after technical certification');
assert.equal(candidate.releaseCandidateFingerprint,null,'RC fingerprint is generated by certification, not self-referential release metadata');
...
assert.equal(candidate.verdicts.technical,'PENDING_VALIDATION');
assert.equal(candidate.verdicts.product,null);
assert.equal(candidate.verdicts.production,'PENDING_MAIN_DEPLOYMENT');
assert.equal(candidate.productionVerification.status,'pending-main-deployment');
...
console.log(`v1.1.0 release-candidate contract passed: ${course.units.length} units, 150 canonical checkpoints, 100 unscored retrieval items, ${candidate.knownV1Blockers.length} release blockers.`);
