...
const pkg=require('../publication-package-engine.js');
...
profile=engine.updateProfile(profile.id,{wordLimit:1000,marginsMm:{left:35},outputs:{pdf:false,ris:true}});assert.equal(profile.marginsMm.left,35);assert.equal(profile.outputs.pdf,false);assert.equal(profile.outputs.ris,true);
...
const publicationProfile={id:'profile.1',name:'Publication',placement:'footnotes',notesHeading:'Notes',includeBibliography:true,document:{title:'John 1 Study',author:'Researcher',date:'2026-08-23'}};
...
(async()=>{
  ...
  const engineSource=fs.readFileSync('publication-package-engine.js','utf8'),ui=fs.readFileSync('publication-package-ui.js','utf8'),html=fs.readFileSync('index.html','utf8'),guard=fs.readFileSync('publication-package-portability.js','utf8'),dpui=fs.readFileSync('data-portability-ui.js','utf8');
  for(const forbidden of ['recordEvidence','recordExposure','recordHint']){assert.ok(!engineSource.includes(forbidden),`package engine must not call ${forbidden}`);assert.ok(!ui.includes(forbidden),`package UI must not call ${forbidden}`)}
  for(const forbidden of ['authorityScore','qualityScore','truthScore','consensusScore','readinessScore'])assert.ok(!engineSource.includes(forbidden),`package engine must not emit ${forbidden}`);
  for(const asset of ['publication-package.css','publication-package-engine.js','publication-package-ui.js','publication-package-portability.js'])assert.ok(html.includes(asset),`${asset} missing from index`);
  assert.ok(html.indexOf('publication-engine.js')<html.indexOf('publication-package-engine.js'));assert.ok(html.indexOf('publication-ui.js')<html.indexOf('publication-package-ui.js'));assert.ok(html.indexOf('publication-portability.js')<html.indexOf('publication-package-portability.js'));
  ...
  console.log('V1.12 submission profiles, preflight severities, OOXML DOCX, semantic notes, PDF container, package manifest, ZIP delivery, mastery firewall, and backup integration: PASS');
