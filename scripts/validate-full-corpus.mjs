...
assert(manifest.coverage.verses===7957,'NT verse coverage changed unexpectedly');
...
for(const meta of manifest.books){
  ...
  for(const [chapter,verses] of Object.entries(book.chapters)){
    ...
    for(const [verse,tokens] of Object.entries(verses)){
      assert(tokens.length>0,`${meta.id} ${chapter}:${verse}: empty verse`);
...
console.log(`BG6 full-corpus validation passed: ${manifest.coverage.tokens} tokens across 27 books.`);
