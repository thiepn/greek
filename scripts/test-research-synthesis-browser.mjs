...
const backup=await page.evaluate(()=>JSON.parse(window.KoineDataPortability.serializeBackup(localStorage,{appVersion:'old'})));if(backup.appVersion!=='v1.9-feature'||!backup.stores['koine-path-research-synthesis-v1'])throw new Error('V1.9 backup export did not include guarded synthesis state/version.');
