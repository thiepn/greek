...
await page.waitForFunction(()=>window.KOINE_FULL_READER?.manifest?.books?.length===27&&window.KOINE_PUBLICATION_PACKAGE_UI&&window.KOINE_SOURCE_LIBRARY&&window.KOINE_CORPUS_RESEARCH&&window.KOINE_RESEARCH_SYNTHESIS&&window.KOINE_PASSAGE_WORKBENCH,{timeout:18000});
...
await page.getByRole('heading',{name:'Turn the dossier into submission artifacts.',exact:true}).waitFor();
const store=await page.evaluate(()=>window.KOINE_SUBMISSION_PACKAGES.snapshot());if(store.schemaVersion!==1||store.profiles.length!==1||store.profiles[0].name!=='Seminary submission')throw new Error('Submission profile did not persist expected V1.12 state.');
...
const docx=await downloaded('#pkg-docx');if(!docx.name.endsWith('.docx')||docx.bytes[0]!==0x50||docx.bytes[1]!==0x4b)throw new Error('DOCX download is not an OOXML ZIP package.');const docxText=docx.bytes.toString('utf8');for(const required of ['[Content_Types].xml','word/document.xml','word/styles.xml','word/footnotes.xml'])if(!docxText.includes(required))throw new Error(`DOCX download omitted ${required}.`);if(!docxText.includes('w:footnoteReference w:id="1"'))throw new Error('DOCX did not preserve semantic V1.11 footnote references.');
...
await browser.close();
console.log('V1.12 browser submission profiles, preflight, real DOCX/PDF downloads, package ZIP/manifest, backup guard, dossier/source/mastery firewalls, and 320px reflow: PASS');