...
function evidenceSignature(learningEngine){const d=learningEngine?.getDashboard?.()||{};return `${Number(d.mastered)||0}:${Number(d.due)||0}:${Number(d.openRemediation)||0}:${d.currentStage?.id||'S0'}:${d.recommendation?.kind||'none'}:${d.recommendation?.unitId||0}`}
...
function initialState(now=new Date()){return{schemaVersion:SCHEMA_VERSION,createdAt:iso(now),updatedAt:iso(now),preferences:{targetId:'course-complete'},current:null,archive:[]}}
function normalizeState(raw,now){const base=initialState(now);if(raw?.schemaVersion!==SCHEMA_VERSION)return base;return{...base,...raw,preferences:{...base.preferences,...(raw.preferences||{})},current:raw.current&&typeof raw.current==='object'?raw.current:null,archive:Array.isArray(raw.archive)?raw.archive.slice(-20):[]}}
...
function milestoneList(curriculum,learningEngine){
  ...
  const course={id:'course-complete',kind:'course',index:stages.length,title:'50-unit canonical course',mastered:Number(dashboard.mastered)||0,total:Number(dashboard.total)||50,progressPercent:dashboard.total?Math.round(dashboard.mastered/dashboard.total*100):0,complete:Number(dashboard.mastered)===Number(dashboard.total)&&stages.every(s=>s.complete),completedAt:null};
  return[...stages,course];
}
function targetRemainingUnits(milestones,targetId){const target=milestones.find(m=>m.id===targetId)||milestones[milestones.length-1];if(!target)return 0;if(target.kind==='course')return Math.max(0,target.total-target.mastered);return milestones.filter(m=>m.kind==='stage'&&m.index<=target.index).reduce((sum,m)=>sum+Math.max(0,m.total-m.mastered),0)}
...
function masteryCountBetween(learningState,start,end){return Object.values(learningState?.units||{}).filter(u=>u?.masteredAt&&new Date(u.masteredAt)>=start&&new Date(u.masteredAt)<end).length}
function forecastRange({remainingUnits,weeklyTargetMinutes,learningState,sessionState,now=new Date()}={}){
  const remaining=Math.max(0,Math.round(Number(remainingUnits)||0));if(!remaining)return{status:'complete',remainingUnits:0,confidence:'high',method:'canonical target already complete',earliestWeeks:0,latestWeeks:0,earliestDate:iso(now),latestDate:iso(now),notice:'This target is already complete by the current canonical evidence state.'};
  const completedWeeks=completedWeekMinutes(sessionState,now),planned=Math.max(30,Math.round(Number(weeklyTargetMinutes)||125));let weeklyCapacity=planned,fastMpu=HEURISTIC_MINUTES_PER_UNIT.fast,slowMpu=HEURISTIC_MINUTES_PER_UNIT.slow,method='workload heuristic',confidence='low',masteries=0,observedMinutes=0;
  if(completedWeeks.length>=2){weeklyCapacity=Math.max(30,Math.round(completedWeeks.reduce((a,w)=>a+w.minutes,0)/completedWeeks.length));const evidenceStart=new Date(`${completedWeeks[0].key}T00:00:00`),evidenceEnd=startOfWeek(now);masteries=masteryCountBetween(learningState,evidenceStart,evidenceEnd);observedMinutes=completedWeeks.reduce((a,w)=>a+w.minutes,0)}
  if(masteries>=2&&observedMinutes>=60&&completedWeeks.length>=2){const observedMpu=observedMinutes/masteries;fastMpu=Math.max(25,observedMpu*.75);slowMpu=Math.max(fastMpu+10,observedMpu*1.35);method='recent mastery and completed-week active-time evidence';confidence=masteries>=4&&completedWeeks.length>=3?'high':'medium'}
  ...
  return{status:'forecast',remainingUnits:remaining,confidence,method,evidenceWeeks:completedWeeks.length,evidenceMasteries:masteries,weeklyCapacityMinutes:weeklyCapacity,minutesPerUnitRange:[Math.round(fastMpu),Math.round(slowMpu)],earliestWeeks,latestWeeks,earliestDate:iso(addDays(startOfWeek(now),earliestWeeks*7)),latestDate:iso(addDays(startOfWeek(now),latestWeeks*7)),notice:'Forecasts are planning ranges, not promised completion dates. Canonical mastery evidence remains authoritative.'};
}
function previousWeekGap(sessionEngine,profile,now){const prevStart=addDays(startOfWeek(now),-7),prevEnd=startOfWeek(now),history=sessionEngine?.snapshot?.().history||[],prior=history.filter(s=>new Date(s.startedAt)>=prevStart&&new Date(s.startedAt)<prevEnd),target=(Number(profile.sessionMinutes)||25)*(Number(profile.daysPerWeek)||5);if(!prior.length)return{observed:false,targetMinutes:target,actualMinutes:0,unusedMinutes:0};const actual=prior.reduce((n,s)=>n+(Number(s.engagedSeconds)||0)/60,0);return{observed:true,targetMinutes:target,actualMinutes:Math.round(actual),unusedMinutes:Math.max(0,Math.round(target-actual))}}
...
function createWeeklyPlan({curriculum,learningEngine,sessionEngine,profile={},guidancePlan=null,targetId='course-complete',now=new Date(),reason='auto'}={}){
  const minutes=clamp(Math.round(Number(profile.sessionMinutes)||25),10,120),daysPerWeek=clamp(Math.round(Number(profile.daysPerWeek)||5),1,7),targetMinutes=minutes*daysPerWeek,mix=GOAL_MIX[profile.goal]||GOAL_MIX['read-nt'],priorities=distributeMinutes(targetMinutes,mix),milestones=milestoneList(curriculum,learningEngine),milestone=nextMilestone(milestones),selectedTarget=milestones.find(m=>m.id===targetId)||milestones[milestones.length-1],gap=previousWeekGap(sessionEngine,profile,now),dayOffsets=scheduleDays(daysPerWeek),sessions=dayOffsets.map((offset,index)=>{const primary=priorities[index%priorities.length],secondary=priorities[(index+1)%priorities.length];return{id:`${weekKey(now)}-${index+1}`,dayOffset:offset,date:dayKey(addDays(startOfWeek(now),offset)),minutes,focus:[viewLabel(primary.view),viewLabel(secondary.view)],primaryView:primary.view}}),dashboard=learningEngine?.getDashboard?.()||{mastered:0,total:50},learningState=learningEngine?.snapshot?.()||{},sessionState=sessionEngine?.snapshot?.()||{history:[]},courseForecast=forecastRange({remainingUnits:Math.max(0,(Number(dashboard.total)||50)-(Number(dashboard.mastered)||0)),weeklyTargetMinutes:targetMinutes,learningState,sessionState,now}),milestoneForecast=milestone?forecastRange({remainingUnits:targetRemainingUnits(milestones,milestone.id),weeklyTargetMinutes:targetMinutes,learningState,sessionState,now}):null,targetForecast=forecastRange({remainingUnits:targetRemainingUnits(milestones,selectedTarget?.id),weeklyTargetMinutes:targetMinutes,learningState,sessionState,now});
  const backlogPolicy=!gap.observed?'No completed-session evidence exists for last week, so no missed-work debt is inferred.':gap.unusedMinutes>0?`${gap.unusedMinutes} unused minutes from last week are not added to this week's target. Unfinished canonical prerequisites and remediation remain ahead of novelty inside the same ${targetMinutes}-minute capacity.`:'Last week met or exceeded the configured capacity; no work is carried as debt.';
  return{schemaVersion:SCHEMA_VERSION,weekKey:weekKey(now),weekLabel:weekLabel(now),generatedAt:iso(now),reason,profile:{sessionMinutes:minutes,daysPerWeek,goal:profile.goal||'read-nt'},profileSignature:profileSignature(profile),evidenceSignature:evidenceSignature(learningEngine),targetMinutes,targetDays:daysPerWeek,priorities,sessions,milestone,milestones,selectedTarget,targetForecast,courseForecast,milestoneForecast,priorWeek:gap,backlogPolicy,guidanceMode:guidancePlan?.mode||null};
}
class WeeklyPlanningEngine{
  ...
  availableTargets(){return milestoneList(this.curriculum,this.learningEngine).map(m=>({id:m.id,title:m.title,complete:m.complete,kind:m.kind}))}
  getPlan(profile={},guidancePlan=null){const current=this.state.current,needs=!current||current.weekKey!==weekKey(this.clock())||current.profileSignature!==profileSignature(profile)||current.evidenceSignature!==evidenceSignature(this.learningEngine)||current.selectedTarget?.id!==this.state.preferences.targetId;if(needs)return this.recalibrate(profile,guidancePlan,'auto');return clone(current)}
  recalibrate(profile={},guidancePlan=null,reason='manual'){if(this.state.current&&this.state.current.weekKey!==weekKey(this.clock())){this.state.archive.push(this.state.current);if(this.state.archive.length>20)this.state.archive.splice(0,this.state.archive.length-20)}this.state.current=createWeeklyPlan({curriculum:this.curriculum,learningEngine:this.learningEngine,sessionEngine:this.sessionEngine,profile,guidancePlan,targetId:this.state.preferences.targetId,now:this.clock(),reason});this.persist();return clone(this.state.current)}
  setTarget(targetId,profile={},guidancePlan=null){const targets=this.availableTargets();if(!targets.some(t=>t.id===targetId))throw new Error('Unknown canonical forecast target.');this.state.preferences.targetId=targetId;this.persist();return this.recalibrate(profile,guidancePlan,'target-change')}
...
return{STATE_KEY,SCHEMA_VERSION,HEURISTIC_MINUTES_PER_UNIT,GOAL_MIX,MemoryStorage,WeeklyPlanningEngine,createWeeklyPlan,milestoneList,targetRemainingUnits,forecastRange,weekKey,startOfWeek,scheduleDays,profileSignature,evidenceSignature,previousWeekGap};
});
