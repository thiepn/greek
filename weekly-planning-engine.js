...
function createWeeklyPlan({curriculum,learningEngine,sessionEngine,profile={},guidancePlan=null,now=new Date(),reason='auto'}={}){
  ...
  const backlogPolicy=!gap.observed?'No completed-session evidence exists for last week, so no missed-work debt is inferred.':gap.unusedMinutes>0?`${gap.unusedMinutes} unused minutes from last week are not added to this week's target. Earlier priorities stay ahead of novelty within the same ${targetMinutes}-minute capacity.`:'Last week met or exceeded the configured capacity; no work is carried as debt.';
  return{schemaVersion:SCHEMA_VERSION,weekKey:weekKey(now),weekLabel:weekLabel(now),generatedAt:iso(now),reason,profile:{sessionMinutes:minutes,daysPerWeek,goal:profile.goal||'read-nt'},profileSignature:profileSignature(profile),targetMinutes,targetDays:daysPerWeek,priorities,sessions,milestone,milestones,courseForecast,milestoneForecast,priorWeek:gap,backlogPolicy,guidanceMode:guidancePlan?.mode||null};
}
...
return{STATE_KEY,SCHEMA_VERSION,HEURISTIC_MINUTES_PER_UNIT,GOAL_MIX,MemoryStorage,WeeklyPlanningEngine,createWeeklyPlan,milestoneList,forecastRange,weekKey,startOfWeek,scheduleDays,profileSignature,previousWeekGap};
});
