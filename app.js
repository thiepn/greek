...
// Legacy prototype drills are retained only for backward-compatible state/data behavior.
// BG4 owns the production Morphology workspace, so these controls may not exist in the DOM.
const drills=[
...
const titles={today:['Today','Your next highest-value Greek task.'],learn:['Learn','Structured foundation curriculum.'],drill:['Morphology','Adaptive morphology practice.'],read:['Read','Guided New Testament reading.'],tutor:['Tutor','Socratic grammar assistance.'],review:['Review','Repair weak forms.'],progress:['Progress','Track reading competence.']};
...
function renderLessons(){const list=document.querySelector('#lesson-list'),stage=document.querySelector('#lesson-stage');if(!list||!stage)return;list.style.display='grid';stage.innerHTML='';list.innerHTML=lessons.map((l,i)=>`<article class="lesson ${state.done.includes(l.id)?'done':''}"><span class="lesson-no">${state.done.includes(l.id)?'✓':i+1}</span><div><h3>${l.title}</h3><p>${l.desc}</p></div><button data-open-lesson="${l.id}">${state.done.includes(l.id)?'Review':'Open'} →</button></article>`).join('');list.querySelectorAll('[data-open-lesson]').forEach(b=>b.addEventListener('click',()=>openLesson(b.dataset.openLesson)))}
function openLesson(id){
  const l=lessons.find(x=>x.id===id),list=document.querySelector('#lesson-list'),stage=document.querySelector('#lesson-stage');
  if(!l||!list||!stage)return;
  ...
  stage.querySelectorAll('.answers button').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    if(fb){fb.textContent=ok?'Correct. Evidence recorded.':'Not yet. This error has been added to targeted remediation.';fb.className='feedback '+(ok?'good':'bad');}
...
function hasLegacyDrillDom(){return !!(document.querySelector('#drill-form')&&document.querySelector('#drill-ask')&&document.querySelector('#drill-feedback')&&document.querySelector('#drill-options'));}
function newDrill(){
  if(!hasLegacyDrillDom())return;
  currentDrill=drills[Math.floor(Math.random()*drills.length)];
  document.querySelector('#drill-form').textContent=currentDrill.form;document.querySelector('#drill-ask').textContent=currentDrill.ask;document.querySelector('#drill-feedback').textContent='';document.querySelector('#drill-feedback').className='feedback';document.querySelector('#drill-options').innerHTML=currentDrill.opts.map(o=>`<button>${o}</button>`).join('');document.querySelectorAll('#drill-options button').forEach(b=>b.addEventListener('click',()=>answerDrill(b.textContent)));
}
function answerDrill(value){
  ...
  const fb=document.querySelector('#drill-feedback');if(fb){fb.textContent=(ok?'Correct. ':'Incorrect. ')+currentDrill.why;fb.className='feedback '+(ok?'good':'bad');}save();
}
...
document.querySelector('#show-translation')?.addEventListener('click',()=>{learning.recordHint({unitId:16,itemId:'John.1.1',level:'full',source:'translation-reveal'});const translation=document.querySelector('#translation');if(translation)translation.hidden=false;});
...
function sendTutor(){const input=document.querySelector('#tutor-input'),chat=document.querySelector('#chat');if(!input||!chat)return;const q=input.value.trim();if(!q)return;chat.insertAdjacentHTML('beforeend',`<div class="bubble user">${escapeHtml(q)}</div><div class="bubble ai">${tutorReply(q)}</div>`);input.value=''}
...
function renderProgress(){
  ...
  const values={'#p-mastery':mastery+'%','#p-attempts':state.attempts,'#p-correct':state.correct,'#p-reading':reading+'%'};
  Object.entries(values).forEach(([selector,value])=>{const el=document.querySelector(selector);if(el)el.textContent=value;});
  const focus=dash.recommendation.unit,competencies=document.querySelector('#competencies');if(competencies&&focus)competencies.innerHTML=window.KoineLearning.DIMENSIONS.map(d=>`<div class="competency"><strong>${d[0].toUpperCase()+d.slice(1)}</strong><span>${focus.dimensions[d].effective}%</span><small>Unit ${focus.id} · ${focus.title} · ${focus.dimensions[d].evidence}/${focus.dimensions[d].minEvidence} minimum evidence events</small></div>`).join('');
}
function renderToday(){
  ...
  const values={'#m-lessons':`${state.done.length} / ${lessons.length}`,'#m-accuracy':accuracy,'#m-review':dash.due+dash.openRemediation,'#m-words':state.words.length,'#side-pct':dash.overall+'%','#total-attempts':state.attempts,'#total-correct':state.correct,'#session-accuracy':sessionAttempts?Math.round(sessionCorrect/sessionAttempts*100)+'%':'—','#next-title':r.title,'#next-text':r.reason};
  Object.entries(values).forEach(([selector,value])=>{const el=document.querySelector(selector);if(el)el.textContent=value;});
  const sideBar=document.querySelector('#side-bar');if(sideBar)sideBar.style.width=dash.overall+'%';
  const next=document.querySelector('#next-btn');if(next)next.onclick=()=>{if(r.kind==='remediation'||r.kind==='review')openView('review');else if([1,5,7,12].includes(r.unitId))openView('learn');else if(r.unitId===16)openView('read');else openView('progress')};
}
